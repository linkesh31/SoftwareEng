...
def back_to_home(patient_id, patient_fullname):
    root.destroy()
    os.system(
        f'python "C:/Users/user/Documents/GitHub/SoftwareEng/Software_Project/Harvind/patienthome.py" {patient_id} {patient_fullname}')
...
def create_patient_profile_window(patient_id, patient_fullname):
    ...
    edit_button = ctk.CTkButton(profile_frame, text="Edit Profile", font=("Arial", 12),
                                command=lambda: edit_profile_action(root, patient_id, patient_fullname))
    edit_button.grid(row=len(labels) + 1, columnspan=2, pady=10)

    back_button = ctk.CTkButton(profile_frame, text="Back", font=("Arial", 12),
                                command=lambda: back_to_home(patient_id, patient_fullname))
